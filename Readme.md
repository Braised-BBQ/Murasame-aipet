...
- **長期記憶與排程**：採用自行架構的長期記憶框架，對於本地系統輕量，長期記憶拓譜和關聯進行了特別優化(詳情請參考開發者文檔中"記憶架構")。
...
- 在執行setup啟動時，會自動建置環境與下載依賴，在最後請確認是否有如下進度條(特別注意總項數125):
 ```
...
### 當前限制
- 目前僅支援 Windows 平台。
- 寵物可能會被全螢幕或防作弊保護的表面覆蓋。
- 本地聊天、視覺和 TTS 性能取決於所選型號和硬體。
