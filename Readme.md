...
主要支援的系統是 Windows 10 和 Windows 11，大部分程式碼由gemini提供。
🌟 核心特色
Live2D 互動渲染：基於 PixiJS 與 Web Audio API，實現滑鼠點擊互動、自動眼神跟隨。
...
3. 設定檔配置
請編輯 pet_backend/config.json（或啟動後透過桌面設定頁面修改），填入你的 API 與tts伺服器連線資訊(login_command，password)。
...
5.tts合成伺服器架設
請於autodl租用實例，並導入社區鏡像kuxiaowo/AIpet-Murasame/AIpet-Murasame_GPT-SoVITs:v1.2.2，第一次開機後請至控制台，將專案夾內reference_voices.zip上傳並解壓替代原資料夾即可。

6.如何互動
由右鍵托拽模型，左鍵對話，左鍵小托盤圖示即可開啟使用者設定介面，滑鼠放到頭上並滑動即會觸發摸頭。

7.google日曆掛接(實測有點雞肋)
請至google cloud (https://cloud.google.com/?hl=zh-TW)登入帳號並申請日曆api，取得json檔並改名成credentials.json放入pet_backend中，預設為關閉，若為關閉則不用有該json檔。
...
運行時後端終端監視器:請於啟動時選擇test_start.bat。



版權與許可:

源專案項目:https://github.com/kuxiaowo/AIpet-Murasame
